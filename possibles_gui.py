...
class SudokuBoard:
    ...
    def clear(self):
        ...
        Empty the board.
        """
        self.grid = [[0 for _ in range(9)] for _ in range(9)]  # [[0 for x in range(9)] for y in range(9)]
        self.locked = []
...
def sudogen_1(_board):
    ...
    for y in range(0, 9, 3):
        for x in range(0, 9, 3):
            ...
            try:
                _board.set(random.randint(x, x+2), random.randint(y, y+2), i, lock=True)
            except ValueError:
...
class SudokuGUI(Frame):
    ...
    def load_game(self):
        def _load_game(filename):
            with open(filename, 'rb') as f:
                ...
                if not isinstance(_board, SudokuBoard):
    ...
    def make_grid(self):
        c = Canvas(self, bg=rgb(128, 128, 128), width=self.width, height=self.height)
        c.pack(side='top', fill='both', expand='1')

        self.rects = [[None for _ in range(9)] for _ in range(9)]       # [[None for x in range(9)] for y in range(9)]
        self.handles = [[None for _ in range(9)] for _ in range(9)]
        rsize = self.rsize
        guidesize = self.guidesize

        for y in range(9):
            for x in range(9):
                (xr, yr) = (x*guidesize, y*guidesize)
                self.rects[y][x] = c.create_rectangle(xr, yr, xr+guidesize, yr+guidesize, width=3)
    ...
    def __init__(self, master, _board):
